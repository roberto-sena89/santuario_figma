    ...
    <div className="space-y-2.5">
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-8 rounded-full border border-border/50 bg-card/60 pl-10 pr-9 text-[13px] font-medium text-foreground placeholder:text-foreground/35 focus:border-gold/60 focus:ring-1 focus:ring-gold/20 focus:bg-card focus:outline-none transition-all"
          />
          {busca && (
            <button
              type="button"
        ...
        <div className="relative">
          <select
            ...
            className="h-9 appearance-none rounded-full border border-border/60 bg-card pl-3.5 pr-8 text-[13px] font-semibold tracking-wide text-foreground shadow-sm focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none transition-all hover:border-gold/25 hover:shadow"
          >
        ...
        <div className="flex items-center rounded-full border border-border/40 bg-transparent p-0.5">
          <button type="button" onClick={() => setOrdem("sel")} aria-pressed={ordem==="sel"} className={`rounded-full px-2 py-1 text-[11px] font-medium transition-colors ${ordem==="sel" ? "text-gold underline underline-offset-4 decoration-2" : "text-foreground/35 hover:text-foreground/60"}`} title="Selecionados primeiro">✓</button>
          <button type="button" onClick={() => setOrdem("az")} aria-pressed={ordem==="az"} className={`rounded-full px-2 py-1 text-[11px] font-medium transition-colors ${ordem==="az" ? "text-gold underline underline-offset-4 decoration-2" : "text-foreground/35 hover:text-foreground/60"}`} title="Ordem alfabética">A–Z</button>
        </div>
      </div>
      ...
      {filtrados.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/40 bg-transparent px-4 py-6 text-center">
          ...
          {podeAdicionar && (
            <button type="button" onClick={() => { onAddPessoa(busca.trim()); setBusca(""); }} className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-gold hover:text-gold-hover transition-colors">＋ Adicionar "{busca.trim()}"</button>
          )}
        </div>
      ) : (
          ...
          {filtrados.map((p) => {
            ...
            return (
              <button
                ...
                aria-pressed={ativo}
                className={`rounded-full px-3 py-1.5 text-xs font-medium tracking-wide transition-colors focus:outline-none ${ativo ? "bg-gold text-gray-900" : "bg-transparent text-foreground/55 hover:bg-white/[0.06] hover:text-foreground/80 border border-transparent hover:border-white/5"}`}
              >
                {ativo ? "✓ " : ""}{p}
              </button>
            );
...
function CultosManager({ onCultosChange }: { onCultosChange: () => void }) {
  ...
  return (
    <div className="mb-6 rounded-[24px] border border-gold/15 bg-gradient-to-br from-card/95 via-card to-card/70 p-6 sm:p-7 shadow-[0_8px_40px_-16px_rgba(212,162,76,0.18),0_4px_16px_rgba(0,0,0,0.2)] backdrop-blur-sm">
      <div className="mb-6 flex items-center gap-4">
        <span className="grid h-11 w-11 flex-shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-gold/20 to-gold/10 text-[20px] shadow-sm shadow-gold/15 ring-1 ring-gold/20" aria-hidden="true">⛪</span>
        ...
        <span className="hidden sm:inline-flex items-center rounded-full bg-gold px-3.5 py-1.5 text-xs font-bold tracking-wide text-gray-900 shadow-sm shadow-gold/20">{cultos.length} {cultos.length === 1 ? "evento" : "eventos"}</span>
      </div>

      {/* Tabela */}
      <div className="mb-5 overflow-hidden rounded-2xl border border-border/60 bg-background/40 shadow-sm">
        <div className="flex items-center gap-3 border-b border-gold/15 bg-gradient-to-r from-gold/[0.09] via-gold/[0.04] to-transparent px-4 py-3">
          <div className="relative flex-1">
            <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-foreground/40 text-[13px]" aria-hidden="true">⌕</span>
            <input value={filtro} onChange={(e) => setFiltro(e.target.value)} placeholder="Filtrar culto (ex: Família)" aria-label="Filtrar cultos" className="w-full h-9 rounded-full border border-border/60 bg-card pl-10 pr-9 text-[13px] font-medium text-foreground placeholder:text-foreground/40 focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none transition-all" />
        ...
        <div className="divide-y divide-border/50">
          {/* header */}
          <div className="hidden sm:grid grid-cols-[1.25fr_96px_1.7fr_148px] gap-3 rounded-t-xl border-b border-gold/20 bg-gradient-to-r from-gold/[0.14] via-gold/[0.08] to-gold/[0.04] px-4 py-3.5 text-[11px] font-bold uppercase tracking-widest text-foreground/65 shadow-sm">
      ...
      {/* Form criar/editar */}
      <form onSubmit={handleSubmit} className="space-y-2.5 rounded-xl border border-gold/15 bg-gradient-to-r from-gold/[0.09] via-gold/[0.04] to-transparent p-3.5 shadow-sm shadow-gold/5">
        <div className="flex flex-col sm:flex-row gap-2 items-end">
          <label className="flex-1 min-w-0 space-y-1 w-full">
            ...
            <select value={diaKey} onChange={(e) => setDiaKey(e.target.value)} className="w-full h-9 rounded-full border border-border/40 bg-card/60 px-3.5 text-[13px] font-medium text-foreground focus:border-gold/60 focus:ring-1 focus:ring-gold/20 focus:bg-card focus:outline-none transition-all">
          ...
          <label className="flex-[1.6] min-w-0 space-y-1 w-full">
            ...
            <select value={titulo} onChange={(e) => setTitulo(e.target.value)} className="w-full h-9 rounded-full border border-border/40 bg-card/60 px-3.5 text-[13px] font-medium text-foreground focus:border-gold/60 focus:ring-1 focus:ring-gold/20 focus:bg-card focus:outline-none transition-all">
              <option value="">— Escolha —</option>
              {SUGESTOES_CULTO.map((s) => (<option key={s} value={s}>{s}</option>))}
            </select>
          </label>
          <div className="flex gap-1.5 shrink-0 w-full sm:w-auto">
            <button type="submit" disabled={!titulo.trim()} className="flex-1 sm:flex-none h-9 rounded-full bg-gold px-5 text-xs font-bold tracking-wide text-gray-900 shadow-sm hover:bg-gold-hover disabled:opacity-30 disabled:cursor-not-allowed transition-colors">{editIndex !== null ? "Salvar" : "＋ Criar"}</button>
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {SUGESTOES_CULTO.map((s) => (
            <button key={s} type="button" onClick={() => setTitulo(s)} className={`rounded-full px-2.5 py-1 text-[11px] font-medium tracking-wide transition-colors ${normalize(titulo) === normalize(s) ? "bg-gold text-gray-900" : "bg-transparent text-foreground/45 hover:bg-white/[0.06] hover:text-foreground/75 border border-transparent hover:border-white/5"}`}>{s}</button>
          ))}
        </div>
      </form>
        ...
        <div className="mx-auto max-w-md px-4 py-16">
          <div className="rounded-2xl border border-border bg-card/80 backdrop-blur-sm p-8 shadow-sm">
            <div className="mb-6 text-center">
              <span className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-full bg-gold/15 text-2xl ring-1 ring-gold/30">
                🔐
              </span>
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-gold focus:ring-2 focus:ring-gold/25 focus:outline-none"
                />
                {erro && (
              ...
              <button
                type="submit"
                className="w-full rounded-xl bg-gold px-4 py-3 text-sm font-semibold text-gray-900 transition-all duration-300 hover:-translate-y-0.5 hover:bg-gold-hover hover:shadow-lg hover:shadow-gold/30"
              >
                Entrar
              </button>
            </form>
  ...
  return (
    <main id="main-content" className="min-h-screen bg-background pt-16">
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        {/* Cabeçalho */}
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4 rounded-[20px] border border-gold/10 bg-gradient-to-br from-card/80 to-card/40 px-5 py-4 sm:px-6 sm:py-5 shadow-[0_4px_24px_rgba(0,0,0,0.15)] backdrop-blur-sm">
          <div className="flex items-center gap-4">
            <span className="grid h-12 w-12 flex-shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-gold/20 to-gold/10 text-[22px] shadow-sm shadow-gold/15 ring-1 ring-gold/20">
              🗓️
            </span>
          ...
          <button
            onClick={onSair}
            className="inline-flex items-center gap-1.5 h-9 rounded-full border border-border/60 bg-card px-5 text-xs font-semibold tracking-wide text-foreground/70 shadow-sm hover:bg-muted hover:text-foreground hover:border-gold/20 hover:shadow transition-all"
          >
            ↪ Sair
          </button>
        </div>
        ...
        <div className="mb-6 rounded-[20px] border border-gold/10 bg-gradient-to-br from-card/80 to-card/40 p-4 sm:p-5 shadow-[0_4px_24px_rgba(0,0,0,0.12)] backdrop-blur-sm">
          <div className="mb-3 flex items-center gap-2 rounded-xl border border-gold/15 bg-gradient-to-r from-gold/[0.11] via-gold/[0.06] to-transparent px-3 py-2.5 shadow-sm shadow-gold/5">
            <span className="grid h-7 w-7 place-items-center rounded-lg bg-gold/15 text-xs ring-1 ring-gold/15">📅</span>
            <span className="text-xs font-bold tracking-widest uppercase text-foreground/55">Semanas</span>
            <span className="ml-auto inline-flex items-center rounded-full bg-gold/10 border border-gold/15 px-2.5 py-1 text-xs font-semibold text-foreground/70">{formatSemana(semana)}</span>
          </div>
            ...
            {semanas.map((s) => (
              <button
                ...
                className={`rounded-full border px-3.5 py-2 text-xs font-semibold tracking-wide transition-all ${
                  s === semana
                    ? "bg-gold border-gold text-gray-900 shadow-sm shadow-gold/25 scale-[1.02]"
                    : "bg-card border-border/60 text-foreground/70 hover:bg-muted hover:text-foreground hover:border-gold/20 hover:shadow-sm"
                }`}
              >
                {formatSemana(s)}
              </button>
          ...
          {([
            ...
            return (
              <button
                ...
                aria-selected={ativo}
                className={`flex-1 inline-flex items-center justify-center gap-1.5 rounded-full px-4 py-2.5 text-xs font-bold tracking-wide transition-all ${ativo ? "bg-gold text-gray-900 shadow-sm shadow-gold/20" : "text-foreground/60 hover:text-foreground hover:bg-muted"}`}
              >
        ...
        <div className="space-y-5">
          {(() => {
            ...
            <div
              key={diaLabel}
              className="rounded-[22px] border border-gold/10 bg-gradient-to-br from-card/90 via-card to-card/60 p-6 sm:p-7 shadow-[0_4px_24px_rgba(0,0,0,0.12)] backdrop-blur-sm"
            >
              <div className="mb-6 flex items-center gap-4">
                <span className="grid h-12 w-12 flex-shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-gold/20 to-gold/10 text-[22px] shadow-sm shadow-gold/15 ring-1 ring-gold/20">
                  {EMOJI_DIA[baseKey] ?? "📅"}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 inline-flex items-center gap-1.5 rounded-full bg-gold/10 border border-gold/15 px-3 py-1 text-xs font-semibold tracking-wide text-foreground/70">
              ...
              <div className="space-y-6 divide-y divide-border/40">
                ...
                <div key={dia.key} className="pt-5 first:pt-0">
                  <div className="mb-4 flex items-center gap-2">
                    <span className="inline-flex items-center rounded-full bg-gold/10 border border-gold/20 px-3 py-1 text-xs font-bold tracking-wide text-gold-deep dark:text-gold-light">{dia.horario}</span>
                    <span className="text-[13px] font-semibold text-foreground/80 truncate">{dia.titulo}</span>
                    <button type="button" onClick={() => setAba("cultos")} className="ml-auto text-[11px] font-medium text-gold hover:text-gold-hover transition-colors">✎ editar</button>
                  </div>
                        ...
                        return (
                          <div key={papel.key}>
                            <div className="mb-2.5 flex items-center gap-2.5">
                              <span className="grid h-8 w-8 place-items-center rounded-xl bg-gold/10 text-[13px] ring-1 ring-gold/15 flex-shrink-0" aria-hidden="true">
                            ...
                            <select
                              ...
                              className="w-full h-9 rounded-full border border-border/60 bg-card px-3.5 text-[13px] font-medium text-foreground shadow-sm focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none hover:border-gold/20 transition-all"
                            >
                          ...
                          return (
                            <div key={papel.key}>
                              <div className="mb-2.5 flex items-center gap-2.5">
                                ...
                                <span className="font-display text-[13px] font-bold tracking-wide text-foreground/85">{papel.label}</span>
                                <span className={`ml-auto rounded-full border px-2.5 py-1 text-[11px] font-bold tracking-wide transition-all ${lista.length > 0 ? "bg-gold border-gold text-gray-900 shadow-sm shadow-gold/20" : "bg-card border-border/60 text-foreground/45"}`}>
          ...
          <button
            ...
            className={`w-full rounded-xl px-4 py-3.5 text-sm font-semibold shadow-lg transition-all duration-300 hover:-translate-y-0.5 ${
              salvo
                ...
                : "bg-gold text-gray-900 shadow-gold/30 hover:bg-gold-hover"
            }`}
          >
        ...
        {aba === "pessoas" && (
        <>
        <div className="rounded-[24px] border border-gold/15 bg-gradient-to-br from-card/95 via-card to-card/70 p-6 sm:p-7 shadow-[0_8px_40px_-16px_rgba(212,162,76,0.18),0_4px_16px_rgba(0,0,0,0.2)] backdrop-blur-sm">
          <div className="mb-6 flex items-center gap-4 rounded-2xl border border-gold/20 bg-gradient-to-r from-gold/[0.11] via-gold/[0.06] to-transparent px-4 py-3.5 shadow-sm shadow-gold/10">
            <span className="grid h-11 w-11 flex-shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-gold/25 to-gold/15 text-[20px] shadow-sm shadow-gold/15 ring-1 ring-gold/25" aria-hidden="true">
              👥
            </span>
            <div className="min-w-0 flex-1">
            ...
            <span className="hidden sm:inline-flex items-center rounded-full bg-gold px-3.5 py-1.5 text-xs font-bold tracking-wide text-gray-900 shadow-sm shadow-gold/20">
              {pessoas.length} {pessoas.length === 1 ? "pessoa" : "pessoas"}
            </span>
          </div>
          <div className="mb-4 rounded-xl border border-gold/15 bg-gold/[0.06] p-2.5">
            <form onSubmit={adicionarPessoa} className="flex gap-2">
              <input
                ...
                className="flex-1 h-9 rounded-full border border-border/60 bg-card px-4 text-[13px] font-medium text-foreground placeholder:text-foreground/40 focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none transition-all"
              />
              <button
                type="submit"
                className="inline-flex items-center gap-1.5 h-9 rounded-full bg-gold px-5 text-[13px] font-bold tracking-wide text-gray-900 shadow-sm shadow-gold/20 transition-all duration-200 hover:bg-gold-hover hover:shadow-md hover:shadow-gold/25 active:scale-[0.98]"
              >
          ...
          {(() => {
            ...
            return (
              <>
                <div className="mb-3 flex gap-2 rounded-xl border border-gold/15 bg-gold/[0.08] p-2.5 shadow-sm shadow-gold/5">
                  <div className="relative flex-1">
                    <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-foreground/40 text-[13px]" aria-hidden="true">⌕</span>
                    <input
                      value={buscaPessoas}
                      ...
                      className="w-full h-9 rounded-full border border-border/60 bg-card pl-10 pr-9 text-[13px] font-medium text-foreground placeholder:text-foreground/40 focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none transition-all"
                    />
                    {buscaPessoas && (
                  ...
                  <div className="flex rounded-full border border-border/60 bg-card p-1">
                    <button type="button" onClick={() => setOrdemPessoas("az")} aria-pressed={ordemPessoas === "az"} className={`rounded-full px-3 py-1 text-xs font-semibold tracking-wide transition-all ${ordemPessoas === "az" ? "bg-gold text-gray-900 shadow-sm" : "text-foreground/60 hover:text-foreground"}`}>A–Z</button>
                    <button type="button" onClick={() => setOrdemPessoas("orig")} aria-pressed={ordemPessoas === "orig"} className={`rounded-full px-3 py-1 text-xs font-semibold tracking-wide transition-all ${ordemPessoas === "orig" ? "bg-gold text-gray-900 shadow-sm" : "text-foreground/60 hover:text-foreground"}`}>Cadastro</button>
                  </div>
                </div>
                <div className="mb-2 flex items-center justify-between px-1">
                  ...
                  {(buscaPessoas || filtroCargoPessoas !== "todos") && (
                    <button type="button" onClick={() => { setBuscaPessoas(""); setFiltroCargoPessoas("todos"); }} className="text-xs font-medium text-foreground/50 hover:text-gold transition-colors">Limpar filtros</button>
                  )}
                </div>
                <div className="mb-3 flex flex-wrap gap-2 rounded-xl border border-gold/15 bg-gold/[0.06] px-3 py-2.5">
                  {([ "todos", "obreiros", "diacono", "presbitero", "pastor"] as const).map((c) => {
                    ...
                    return (
                      <button key={c} type="button" onClick={() => setFiltroCargoPessoas(c)} aria-pressed={ativo} className={`rounded-full border px-3.5 py-1.5 text-xs font-semibold tracking-wide transition-all ${ativo ? "bg-gold border-gold text-gray-900 shadow-sm shadow-gold/20" : "bg-card border-border/60 text-foreground/65 hover:bg-muted hover:text-foreground hover:border-gold/15"}`}>
                ...
                {ordenadas.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {ordenadas.map((p) => (
                      <span key={p} className="group inline-flex items-center gap-2 rounded-full border border-border/60 bg-card px-3.5 py-2 text-xs font-medium text-foreground/80 transition-all duration-200 hover:border-gold/25 hover:bg-gold/[0.06] hover:text-foreground">
                        <span className="h-1.5 w-1.5 rounded-full bg-gold/60 shadow-sm shadow-gold/20 flex-shrink-0" aria-hidden="true" />
                        {p}
          ...
          <p className="mt-5 flex items-start gap-2 rounded-xl bg-gold/[0.06] border border-gold/10 px-3.5 py-2.5 text-xs leading-relaxed text-foreground/60">
          ...
          <div className="mt-4 rounded-xl border border-gold/20 bg-gradient-to-br from-gold/[0.07] to-transparent p-3.5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              }} id="btn-publicar-pessoas" className="inline-flex items-center gap-1.5 h-8 rounded-full bg-gold px-5 text-xs font-bold text-gray-900 shadow-sm hover:bg-gold-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed">🚀 Publicar no GitHub → Vercel</button>
              <button type="button" onClick={async () => {
              ...
              }} className="inline-flex items-center gap-1.5 h-8 rounded-full border border-border/60 bg-card px-4 text-xs font-semibold text-foreground/70 hover:border-gold/30 hover:text-foreground transition-colors">📋 Copiar</button>
              ...
              }} className="inline-flex items-center gap-1.5 h-8 rounded-full border border-border/60 bg-card px-4 text-xs font-semibold text-foreground/70 hover:border-gold/30 hover:text-foreground transition-colors">⬇ JSON</button>
            </div>
          </div>
        </div>
        </>
